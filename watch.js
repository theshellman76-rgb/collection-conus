    ...
    try {
      ...
      (data.itemSummaries || []).forEach(item => {
        ...
        results.push({
          ...
          url: item.itemWebUrl,
          _ebayItemId: item.itemId,
          _ebayMarketplace: marketplace,
        });
      });
    } catch (e) {
...
// Description complète d'une annonce eBay (appel supplémentaire, un par annonce candidate)
async function fetchEbayDescription(token, ebayItemId, marketplace) {
  try {
    const res = await fetch(`https://api.ebay.com/buy/browse/v1/item/${encodeURIComponent(ebayItemId)}`, {
      headers: {
        "Authorization": `Bearer ${token}`,
        "X-EBAY-C-MARKETPLACE-ID": marketplace,
      },
...
// Description complète d'une annonce shellauction.net (page individuelle de l'annonce)
async function fetchShellauctionDescription(url) {
  try {
    const res = await fetch(url, { headers: { "User-Agent": "Mozilla/5.0 (compatible; ConusWishlistWatch/1.0)" } });
    if (!res.ok) return "";
...
async function main() {
  ...
  for (const item of wishlist) {
    ...
    // Taille et qualité : extraction "best effort", d'abord depuis le titre (pas de champ
    // structuré chez ces vendeurs). Si le titre ne suffit pas à évaluer un critère demandé,
    // on va lire la description complète de l'annonce (coûte un appel de plus par annonce
    // candidate, donc seulement fait quand c'est vraiment nécessaire).
    found = found.map(f => ({
      ...f,
    ...
    const needsMoreInfo = f =>
      (item.tailleMin != null && f.tailleVue == null) ||
      (item.qualiteMin && !f.qualiteVue);

    for (const f of found) {
      if (!needsMoreInfo(f)) continue;
      let description = "";
      if (f.source === "eBay" && ebayToken && f._ebayItemId) {
        description = await fetchEbayDescription(ebayToken, f._ebayItemId, f._ebayMarketplace);
      } else if (f.source === "shellauction.net") {
        description = await fetchShellauctionDescription(f.url);
      }
      if (description) {
        const fullText = f.title + " " + description;
        if (f.tailleVue == null) f.tailleVue = extractSizeFromTitle(fullText);
        if (!f.qualiteVue) f.qualiteVue = extractQualityFromTitle(fullText);
      }
    }

    // En l'absence d'info repérable (même après lecture de la description), on laisse
    // passer l'annonce plutôt que de risquer de rater une bonne trouvaille.
    found = found.filter(f => {
    ...
    found.forEach(f => {
      const { _ebayItemId, _ebayMarketplace, ...clean } = f;
      allMatches.push({ ...clean, species: item.nom, limit: item.prixLimite });
    });
  }
